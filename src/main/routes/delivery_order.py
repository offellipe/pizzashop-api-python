...
@orders_blueprint.route("/orders/<string:order_id>/deliver", methods=["PATCH"])
@authenticate_user  # Middleware para autenticação
def deliver_order(order_id):
    """
    Marca um pedido como 'delivered'.
    """
    try:
        ...
        if order.status != "delivering":
            return jsonify({"message": "O pedido já foi entregue."}), 400

        # Atualizar o status do pedido para 'delivered'
        order_repo.update_order_status(order_id, "delivered")
